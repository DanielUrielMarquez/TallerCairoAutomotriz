...
router.post("/", (req, res) => {
  ...
  if (!vehiculoExiste || !trabajadorExiste) {
    return res.status(400).json({ error: "vehiculoId o trabajadorId invalido" });
  }
...
router.patch("/:id/estado", (req, res) => {
  ...
  const permitidos = ["abierta", "en_proceso", "cerrada"];
  if (!permitidos.includes(estado)) return res.status(400).json({ error: "estado invalido" });
